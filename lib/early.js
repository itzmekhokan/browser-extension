 ...
 *   - The user's browser-wide preference is to hide the admin bar
 *     (default is visible).
 *
...
(async function () {
  ...
  try {
    ...
    const allPrefs = prefsData.wp_preferences_v1 || {};

    const isKnownWP = entry && entry.isWordPress;
    const shouldHide = globalThis.WPPrefs.isAdminBarHidden(allPrefs);
